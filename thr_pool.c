/*
 * Borrowed from Oracle opensource
 *
...
worker_cleanup(void *p)
{
        thr_pool_t *pool = (thr_pool_t *)p;
	--pool->pool_nthreads;
...
thr_pool_t *
thr_pool_create(uint32_t min_threads, uint32_t max_threads, uint32_t linger,
	pthread_attr_t *attr)
{
	thr_pool_t	*pool;
...
thr_pool_wait(thr_pool_t *pool)
{
	...
	pthread_cleanup_push(mutex_unlock, &pool->pool_mutex);
